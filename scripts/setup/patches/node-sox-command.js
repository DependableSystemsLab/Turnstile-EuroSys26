...
async function input (RED, node, msg, config) {
    ...
    if (!soxPath) {
        try {
            result = await exec('which sox');
            soxPath = true;
        } catch (err) {
            try {
                result = await exec('where.exe sox');
                soxPath = true;
            }
            catch (err) {
                return node.error("Sox was not found. Please intall it and make sure it is part of the PATH.");
            }
        }
    }

    let useSpawn = (config.spawn === "true");
    let command = helper.getContextValue(RED, node, msg, config.cmd, config.cmdType);
